...
Comme on le faisait dans la classe \texttt{Robot}, on doit ajouter les composantes de notre sous-système dans le haut de la classe. Voici un exemple avec un sous-système nommé \texttt{BasePilotable}.

\begin{MyTCB}{Les attributs et le constructeur d'un sous-système}
/**
...
Pour ajouter une commande par défaut à un sous-système, il suffit d'utiliser \texttt{setDefaultCommand} dans la méthode \texttt{initDefaultCommand} (voir l'exemple précédent).

\subsection{Ajouter des méthodes à un sous-système}
Un sous-système n'est pas complet tant qu'on ne lui ajoute pas de méthodes. 
...
\subsubsection{La méthode \texttt{initialise()}}
\subsubsection{La méthode \texttt{execute()}}
\subsubsection{La méthode \texttt{isFinished()}}
\subsubsection{La méthode \texttt{end()}}
\subsubsection{La méthode \texttt{interrupted()}}

\subsubsection{Ajouter un délai avec \texttt{setTimeout(double)}}
...
\subsection{Le processus de création d'une commande}
...
question/.style={rectangle, draw=blue!60, fill=blue!5, very thick, minimum size=7mm, text width=6cm, inner sep=0.25cm, rounded corners}
	...
	\node[question, anchor=north, shift={(0mm,-5mm)}] (n3)  at (n2.south)	{Déterminer les sous-systèmes que la commande va utiliser. Est-ce que je dois en ajouter un nouveau?};
	...
	\node[action, anchor=north, shift={(0mm,-5mm)}] (n6) at (n5.south)	{Ajouter les composantes du sous-système comme attributs privés (\texttt{private}).};
	\node[action, anchor=north, shift={(0mm,-5mm)}] (n7) at (n6.south)	{Instancier (\texttt{new}) ces composantes dans les constructeurs et ajouter les ports nécessaires à la classe \texttt{K}.};
...
\part{Git et les logiciels de gestion de versions}
